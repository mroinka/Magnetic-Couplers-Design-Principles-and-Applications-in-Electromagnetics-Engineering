...
elseif coordinateSystem == 's'
    % محاسبه مختصات مستطیلی
    disp('Enter the spherical coordinate in the format');
    sphericalCoordinate = input('[rho phi theta]... \n > ');
    
    % تنظیم مقدار پیش فرض برای مختصات کروی اگر ورودی خالی باشد
    if isempty(sphericalCoordinate)
        sphericalCoordinate = [0 0 0];
    end
    
    % محاسبه مختصات مستطیلی
    disp('Rectangular coordinates [x y z]:');
    rectangularCoordinate = [sphericalCoordinate(1)*cos(sphericalCoordinate(2))*sin(sphericalCoordinate(3)) sphericalCoordinate(1)*sin(sphericalCoordinate(2))*sin(sphericalCoordinate(3)) sphericalCoordinate(1)*cos(sphericalCoordinate(3))];
    disp(rectangularCoordinate);
    
    % محاسبه مختصات استوانه‌ای
    disp('Cylindrical coordinates [rho phi(rad) z]:');
    cylindricalCoordinate = [sphericalCoordinate(1)*sin(sphericalCoordinate(3)) sphericalCoordinate(2) sphericalCoordinate(1)*cos(sphericalCoordinate(3))];
    disp(cylindricalCoordinate);
